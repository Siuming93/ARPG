...
namespace Assets.LengjingUnity3D.sdk
{
    public class LJTest : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            if (GUILayout.Button("init", GUILayout.Height(100), GUILayout.Width(200)))
            {
            ...
            {
                LJSDK.Instance.pay(this.gameObject.name, "payCallback", 100, "zuanshi", 10, "60钻石", "XXX-XXX-XXX-XXX",
                    "http://callbackurl");
            }
        ...
        private void userCallback(string result)
        {
            ...
            if (LJSDK.SUCCESS == resultCode)
            {
                ...
                LJSDK.Instance.setExtData(LJSDK.enterServer, "13524696", "fangmu", 24, 1, "mutu1qu", 88, 2,
                    "wujintianya");
                LJSDK.Instance.setExtData(LJSDK.createRole, "13524696", "fangmu", 24, 1, "mutu1qu", 88, 2, "wujintianya");
                LJSDK.Instance.setExtData(LJSDK.levelUp, "13524696", "fangmu", 24, 1, "mutu1qu", 88, 2, "wujintianya");
            }
		...
		void onGameExit(){
			...
			//这是调用Android的系统api来杀死进程，游戏可自行选择是否使用
			LJSDK.Instance.androidContext().Call("androidKillProcess");
		}
